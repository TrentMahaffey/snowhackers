-- Create materialized view for observed vs forecast comparison
-- This replaces the slow _OBS_VS_FC_SQL query

-- For 24 hours
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_obs_vs_fc_24h AS
WITH params AS (
  SELECT
    now() - make_interval(hours => 24) AS start_ts,
    now() AS end_ts,
    (CURRENT_DATE - make_interval(days => 1)) AS start_date
),
obs AS (
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_mv_obs_vs_fc_24h_station ON mv_obs_vs_fc_24h(station_triplet);
CREATE INDEX IF NOT EXISTS idx_mv_obs_vs_fc_24h_obs ON mv_obs_vs_fc_24h(obs_cm DESC);
